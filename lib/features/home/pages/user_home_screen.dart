...
class _UserHomeScreenState extends ConsumerState<UserHomeScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final requisitionsAsync = ref.watch(ProviderUtils.requisitions(user!.email!));
    ...
    return userProfileAsync.hasValue
      ...
      body: requisitionsAsync.when(
        data: (requisitions) => _buildTabView(requisitions),
  ...
  Widget _buildTabView(List<Requisition> requisitions) {
    final pendingRequisitions = requisitions.where((req) => !req.authorisedByMD).toList();
    final approvedRequisitions = requisitions.where((req) => req.authorisedByMD).toList();
    final declinedRequisitions = requisitions.where((req) => req.authorisedByMD).toList();
    final cancelledRequisitions = requisitions.where((req) => req.authorisedByMD).toList();
            ...
            child: TabBarView(
              ...
              children: [
                RequisitionTab(
                  ...
                  requisition: pendingRequisitions,
                ),
                RequisitionTab(
                  searchTerm: searchTerm,
                  requisition: approvedRequisitions,
                ),
                RequisitionTab(
                  searchTerm: searchTerm,
                  requisition: declinedRequisitions,
                ),
                RequisitionTab(
                  searchTerm: searchTerm,
                  requisition: cancelledRequisitions,
                ),
              ],
            ),
          ),
